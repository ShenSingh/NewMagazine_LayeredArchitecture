...
public class AddOfficerController extends MainDashBoard  implements Initializable {
    ...
    OfficerBO officerBO = (OfficerBO) BoFactory.getInstance().getBo(BoFactory.BoTypes.OFFICER);
    ...
    private void setNextOfficerId() {
        try {
            List<OfficerDTO> allOfficer = officerBO.getAllOfficer();
    ...
    private void setComboBoxValues() throws SQLException {
        ...
        List<SectionDTO> jailSections = null;
        try {
            jailSections = sectionBO.getAllSection();
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(e);
        }
    ...
    private void setOfficerCount() {
        List<OfficerDTO> allOffisers = new ArrayList<>();

        try {
            allOffisers = officerBO.getAllOfficer();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(e);
    ...
    private String seUniCount(List<OfficerDTO> allOffisers) {
    ...
    public void submitBtn(ActionEvent actionEvent) throws SQLException, ClassNotFoundException {
        ...
        if (checkEmptyFields()){
            ...
            OfficerDTO officer = new OfficerDTO(officerId.getText(),fName.getText(),lName.getText(),java.sql.Date.valueOf(DOB.getValue()),NIC.getText(),gender.getSelectionModel().getSelectedItem(),address.getText(),email.getText(),number.getText(),positionComboBox.getSelectionModel().getSelectedItem(),sectionId.getSelectionModel().getSelectedItem(),Double.parseDouble(salery.getText()));

            if (officerBO.saveOfficer(officer)) {
    ...
    public void salaryOnAction(ActionEvent actionEvent) throws SQLException, ClassNotFoundException {
